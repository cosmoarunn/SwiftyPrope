...
private let propeTempDir = {
    //let dirPath = NSTemporaryDirectory().appendingPathComponent("prope_temp_\(UUID())", conformingTo: .application)
    let dirPath = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("prope_temp_\(UUID())", conformingTo:.application).path
    propePrecondition(PropeFileSystem.ensureDirectoryExists(dirPath, fileProtectionType: .complete))
    return dirPath
}()
...
public func PropeTemporaryDirectory() -> String {
    return propeTempDir
}

public func PropeTemporaryDirectoryAccessibleAfterFirstAuth() -> String {
    let dirPath = NSTemporaryDirectory()
    propePrecondition(PropeFileSystem.ensureDirectoryExists(dirPath, fileProtectionType: .completeUntilFirstUserAuthentication))
    return dirPath
}

private let cleanTmpDispatchQueue = DispatchQueue(label: "org.fgbs.clean-tmp", qos: .utility)
...
private func ClearOldTemporaryDirectoriesSync() {
    ...
    let currentTempDirName = (PropeTemporaryDirectory() as NSString).lastPathComponent
    ...
    do {
        ...
        propeFailDebug("contentsOfDirectoryAtPath error: \(error)")
        return
    }
    for fileName in fileNames {
        ...
        //let filePath: Any = dirPath.appendingPathComponent(fileName, conformingTo: .application)
        let filePath = URL(fileURLWithPath: dirPath).appendingPathComponent("Prope", conformingTo:.application).path
        // Delete files with either:
        //
        // a) "prope_temp" name prefix.
        // b) modified time before app launch time.
        if !fileName.hasPrefix("prope_temp") {
            do {
                let attributes = try FileManager.default.attributesOfItem(atPath: filePath)
                // Don't delete files which were created in the last N minutes.
                let mtime = attributes[.modificationDate] as? Date
                guard let mtime else {
                    PropeLog.error("failed to get a modification date for file or directory at: \(filePath)")
                    continue
                }
                ...
                PropeLog.error("Could not get attributes of file or directory at: \(filePath)")
                continue
            }
        }

        if !PropeFileSystem.deleteFileIfExists(filePath) {
            // This can happen if the app launches before the phone is unlocked.
            // Clean up will occur when app becomes active.
            PropeLog.warn("Could not delete old temp directory: \(filePath)")
        }
    }
}
...
@objc
public class PropeFileSystem: NSObject {
    ...
    private static func protectRecursiveContents(atPath path: String) -> Bool {
        ...
        for relativePath in directoryEnumerator {
            guard let relativePath = relativePath as? String else {
                propeFail("type of elements from FileManager.enumerator was not String")
            }
           // let filePath: Any = dirPath.appendingPathComponent(relativePath, conformingTo: .application)
            
            let filePath = URL(fileURLWithPath: dirPath).appendingPathComponent(relativePath, conformingTo:.application).path
    ...
    public static func protectFileOrFolder(atPath path: String, fileProtectionType: FileProtectionType = .completeUntilFirstUserAuthentication) -> Bool {
        do {
            ...
            propeFailDebug("Could not protect file or folder: \(error)")
            return false
        }
    ...
    public static func appLibraryDirectoryPath() -> String {
        guard let last = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).last else {
            propeFail("no urls returned for the user library directory")
        }
        return last.path
    }
    ...
    private static let cachesDirectoryPathPrecomputed: String = {
        guard let result = NSSearchPathForDirectoriesInDomains(.cachesDirectory, .userDomainMask, true).first else {
            propeFail("no search paths returned for user caches directories")
        }
        return result
    }()
    @objc
    ...
    public static func ensureFileExists(_ filePath: String) -> Bool {
        ...
        propeFailDebug("Failed to create file.")
        return false
    }
    ...
    public static func deleteContents(ofDirectory dirPath: String) {
        do {
            ...
            propeFailDebug("Could not retrieve files in directory.")
        }
    }
    ...
    public static func fileSize(ofPath filePath: String) -> NSNumber? {
        do {
            ...
            guard let result = attrs[.size] as? NSNumber else {
                propeFail("file size attribute was not NSNumber")
            }
            return result
        } catch {
            PropeLog.error("Couldn't fetch file size: \(error)")
            return nil
        }
    }
...
extension PropeFileSystem {
    ...
    fileprivate static func ensureDirectoryExists(_ dirPath: String, fileProtectionType: FileProtectionType) -> Bool {
        do {
            ...
            propeFailDebug("Failed to create directory: \(dirPath), error: \(error)")
            return false
        }
    }
}

public extension PropeFileSystem {
    @objc
    ...
    class func moveFile(from fromUrl: URL, to toUrl: URL) throws {
        guard FileManager.default.fileExists(atPath: fromUrl.path) else {
            throw PropeAssertionError("Source file does not exist.")
        }
        guard !FileManager.default.fileExists(atPath: toUrl.path) else {
            throw PropeAssertionError("Destination file already exists.")
        }
    ...
    class func copyFile(from fromUrl: URL, to toUrl: URL) throws {
        ...
        #if TESTABLE_BUILD
        guard FileManager.default.fileExists(atPath: toUrl.path) else {
            throw PropeAssertionError("Destination file not created.")
        }
        #endif
    }

    class func recursiveFilesInDirectory(_ dirPath: String) throws -> [String] {
        propeAssertDebug(!dirPath.isEmpty)
...
public extension PropeFileSystem {
    ...
    private class func tempDirPath(availableWhileDeviceLocked: Bool) -> String {
        return availableWhileDeviceLocked
            ? PropeTemporaryDirectoryAccessibleAfterFirstAuth()
            : PropeTemporaryDirectory()
    }
}
    ...
    class func deleteFile(_ filePath: String, ignoreIfMissing: Bool = false) -> Bool {
        do {
            ...
            let knownNoWritePermissionUrls = [
                PropeFileSystem.appSharedDataDirectoryURL().appendingPathComponent(".com.apple.mobile_container_manager.metadata.plist")
            ]
            propeAssertDebug(knownNoWritePermissionUrls.contains(attemptedUrl))
            return false
        } catch {
            propeFailDebug("\(error.localizedDescription)")
            return false
        }
    }
}
...
public extension PropeFileSystem {
    ...
    class func freeSpaceInBytes(forPath path: URL) throws -> UInt64 {
        ...
        guard let result = resourceValues.volumeAvailableCapacityForImportantUsage else {
            throw PropeGenericError("Could not determine remaining disk space")
        }
...
public extension PropeFileSystem {
    class func createTempFileSlice(url: URL, start: Int) throws -> (URL, Int) {
        ...
        let dataSliceFileUrl = PropeFileSystem.temporaryFileUrl(isAvailableWhileDeviceLocked: true)
        ...
        guard dataSliceLength + start == entireFileData.count else {
            throw PropeAssertionError("Could not slice the data.")
        }
