...
public enum PropeNavigationBarStyle: Int {
...
public class PropeNavigationBar: UINavigationBar {
    ...
    private var navbarBackgroundColor: UIColor {
        return navbarBackgroundColorOverride ?? .systemBlue //Theme.navbarBackgroundColor
    }

    private var navbarTintColor: UIColor {
        return navbarTintColorOverride ??  .systemBackground // Theme.primaryTextColor
    }

    // MARK: Appearance

    private var style: PropeNavigationBarStyle?
    private var appearance: PropeNavigationBarAppearance?

    internal func setStyle(_ style: PropeNavigationBarStyle, animated: Bool) {
        self.style = style
    ...
    private func updateAppearance(animated: Bool) {
        ...
        let appearance = PropeNavigationBarAppearance.appearance(
            for: style ?? .blur,
...
internal struct PropeNavigationBarAppearance: Equatable {
    ...
    enum TempTheme { case light, dark }
    
    var theme:TempTheme = .light
    
    static func appearance(
        for style: PropeNavigationBarStyle,
        navbarBackgroundColor: UIColor,
        tintColor: UIColor
    ) -> Self {
        var appearance = PropeNavigationBarAppearance()
        appearance.barStyle = .default //  Theme.barStyle
        appearance.tintColor = tintColor
        if UIAccessibility.isReduceTransparencyEnabled {
            appearance.backgroundStyle = .image(navbarBackgroundColor)
        } else {
            appearance.backgroundStyle = .blur(UIBlurEffect(style: .light)) //Theme.barBlurEffect)
        }
        ...
        let applyDarkThemeOverride = {
            ...
            appearance.backgroundStyle = .tint(.systemBlue) // .tint(Theme.darkThemeBackgroundColor)
            appearance.tintColor = .systemBackground // Theme.darkThemePrimaryColor
        }
    ...
    func apply(to navigationBar: PropeNavigationBar) {
    ...
    private var backgroundImage: UIImage? {
        ...
        case .blur:
            return UIImage(ciImage: CIImage(color: .clear))
        case .tint:
            return nil
        case .image(let color):
            return UIImage(ciImage: CIImage(color: CIColor(color: color)))
        }
    }
...
fileprivate extension UIVisualEffectView {
    ...
    func matchBackgroundColor(_ color: UIColor) -> Bool {
        if let tintingView = tintingView {
            ...
            propeFailDebug("Unexpectedly missing visual effect subview")
            return false
        }
    }
}
