//
//  PropeNavigationController.swift
//  PAndE
//
...
public protocol PropeNavigationChildController: AnyObject {

    /// If non-nil, will use the provided child (should be a child view controller) for
    /// all other protocol methods.
    var childForOWSNavigationConfiguration: PropeNavigationChildController? { get }
    ...
    /// Defaults to `blur`.
    var preferredNavigationBarStyle: PropeNavigationBarStyle { get }
...
extension PropeNavigationChildController {

    public var childForOWSNavigationConfiguration: PropeNavigationChildController? { nil }

    public var shouldCancelNavigationBack: Bool { false }

    public var preferredNavigationBarStyle: PropeNavigationBarStyle { .blur }
...
/// unsaved changes.
open class PropeNavigationController: UINavigationController {

    private var owsNavigationBar: PropeNavigationBar {
        return navigationBar as! PropeNavigationBar
    }
    ...
    public override var delegate: UINavigationControllerDelegate? {
        ...
        set {
            if newValue === self {
                propeFailDebug("Self is already the delegate! Override methods instead.")
                return
            }
    ...
    public init() {
        super.init(navigationBarClass: PropeNavigationBar.self, toolbarClass: nil)
    ...
    open override var supportedInterfaceOrientations: UIInterfaceOrientationMask {
        if let delegateOrientations = self.delegate?.navigationControllerSupportedInterfaceOrientations?(self) {
            ...
            return  .portrait //UIDevice.current.defaultSupportedOrientations
        }
    }
    ...
    public override var preferredStatusBarStyle: UIStatusBarStyle {
        ...
        if !CurrentAppContext().isMainApp {
            ...
            return .lightContent // Theme.isDarkThemeEnabled ? .lightContent : .darkContent
        }
    }
...
extension PropeNavigationController: UIGestureRecognizerDelegate {
    ...
    public func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        propeAssertDebug(gestureRecognizer === self.interactivePopGestureRecognizer)
...
extension PropeNavigationController: UINavigationBarDelegate {

    // All PropeNavigationController serve as the UINavigationBarDelegate for their navbar.
    ...
    public func navigationBar(_ navigationBar: UINavigationBar, shouldPop item: UINavigationItem) -> Bool {
        propeAssertDebug(interactivePopGestureRecognizer?.delegate === self)
...
extension PropeNavigationController: UINavigationControllerDelegate {
...
extension UIViewController {
    ...
    func getFinalNavigationChildController() -> PropeNavigationChildController? {
        guard let child = self as? PropeNavigationChildController else { return nil }
...
extension PropeNavigationChildController {
    ...
    func getFinalChild() -> PropeNavigationChildController {
